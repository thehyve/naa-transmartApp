...
function convertCrossNodeToConcept(node, showValueConstraintPopup)
{
	...
	var myConcept=new Concept(name, key, level, tooltip, tablename, dimcode, comment, normalunits, oktousevalues, value, nodeType, inOutCode, timingLevel);
	return myConcept;
}
...
function dropOntoCategorySelectionCrossTrial(targetDiv, dropObject)
{
	...
	else 
	{
		...
		{
			//Add the item to the input.
			var concept = createPanelItemNew(targetdiv, convertCrossNodeToConcept(dropObject, showValueConstraintPopup), shortNameDepth);
		}
		else
		{
			//Add the item to the input.
