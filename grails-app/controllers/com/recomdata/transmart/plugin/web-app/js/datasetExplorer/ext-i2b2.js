...
Ext.ux.OntologyTreeLoader = Ext.extend(Ext.tree.TreeLoader, {
 ...
 //       getChildConceptPatientCounts(node);
        if (node.attributes.level == 1) {
            handleFolderHasFilesRequest(this, response, node, callback);
        }
        else {
...
 function getFileFolderNode (parentNode) {
    ...
    newnode.addListener('contextmenu',fileFolderRightClick);

    return newnode;
}

function fileFolderRightClick (eventNode, event)
{
